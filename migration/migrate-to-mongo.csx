...
Console.WriteLine($"  MySQL : {mysqlConnStr[..Math.Min(60, mysqlConnStr.Length)]}...");
...
async Task<long> UpsertCollection(
    IMongoCollection<BsonDocument> collection,
    List<BsonDocument> docs,
    Func<BsonDocument, FilterDefinition<BsonDocument>> filterFn)
{
    long count = 0;
    foreach (var doc in docs)
    {
        await collection.ReplaceOneAsync(filterFn(doc), doc, new ReplaceOptions { IsUpsert = true });
        count++;
    }
    return count;
}

await using var mysql = new MySqlConnection(mysqlConnStr);
...
var userCount = await UpsertCollection(
    db.GetCollection<BsonDocument>("users"), userDocs,
    d => Builders<BsonDocument>.Filter.Eq("_id", d["_id"]));
Console.WriteLine($"   v {userCount} users migrated (upsert, idempotent)");
...
var topicCount = await UpsertCollection(
    db.GetCollection<BsonDocument>("topics"), topicDocs,
    d => Builders<BsonDocument>.Filter.Eq("_id", d["_id"]));
Console.WriteLine($"   v {topicCount} topics migrated");
...
var ideaCount = await UpsertCollection(
    db.GetCollection<BsonDocument>("ideas"), ideaDocs,
    d => Builders<BsonDocument>.Filter.Eq("_id", d["_id"]));
Console.WriteLine($"   v {ideaCount} ideas migrated");
...
var voteCount = await UpsertCollection(
    db.GetCollection<BsonDocument>("votes"), voteDocs,
    d => Builders<BsonDocument>.Filter.Eq("_id", d["_id"]));
Console.WriteLine($"   v {voteCount} votes migrated");

// Validation
Console.WriteLine();
Console.WriteLine("-- Validating integrity...");

var mysqlUserCount  = await mysql.ExecuteScalarAsync<long>("SELECT COUNT(*) FROM users");
var mysqlTopicCount = await mysql.ExecuteScalarAsync<long>("SELECT COUNT(*) FROM topics");
var mysqlIdeaCount  = await mysql.ExecuteScalarAsync<long>("SELECT COUNT(*) FROM ideas");
var mysqlVoteCount  = await mysql.ExecuteScalarAsync<long>("SELECT COUNT(*) FROM votes");

var mongoUserCount  = await db.GetCollection<BsonDocument>("users").CountDocumentsAsync(FilterDefinition<BsonDocument>.Empty);
var mongoTopicCount = await db.GetCollection<BsonDocument>("topics").CountDocumentsAsync(FilterDefinition<BsonDocument>.Empty);
var mongoIdeaCount  = await db.GetCollection<BsonDocument>("ideas").CountDocumentsAsync(FilterDefinition<BsonDocument>.Empty);
var mongoVoteCount  = await db.GetCollection<BsonDocument>("votes").CountDocumentsAsync(FilterDefinition<BsonDocument>.Empty);
...
Check("users",  mysqlUserCount,  mongoUserCount);
Check("topics", mysqlTopicCount, mongoTopicCount);
Check("ideas",  mysqlIdeaCount,  mongoIdeaCount);
Check("votes",  mysqlVoteCount,  mongoVoteCount);
