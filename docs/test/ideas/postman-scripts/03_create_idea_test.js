...
pm.test("Idea created successfully", function () {
  ...
  pm.collectionVariables.set("idea_ownerId", String(json.ownerId || ""));
});

pm.test("Idea fields match request variables", function () {
  const json = pm.response.json();
  ...
  pm.expect(json.description).to.equal(pm.collectionVariables.get("idea_description"));
...
pm.test("HATEOAS _links are present (topic OPEN => vote link exists)", function () {
  ...
  const selfLink = json._links.find(l => l.rel === "self");
